...
features = data['query'].tolist() #Q 부분만 리스트로 변환
labels = data['intent'].tolist()#label(감정) 부분만 리스트로 변환 0:일상다반사, 1:이별(부정), 2:사랑(긍정)
...
logits = Dense(5, name='logits')(dropout_hidden) # 3이 아닌 5로 해야 함(3으로 줄일 수 없나 봐...)
