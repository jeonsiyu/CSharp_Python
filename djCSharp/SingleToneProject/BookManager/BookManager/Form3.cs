    ...
    public partial class Form3 : Form, IRefresh
    {
        public Form3()
        {
            ...
            button2.Click += delegate (object o, EventArgs e)
            {
                try
                {
                    User u = DataManager.Instance.users.Single(item => item.id.Equals(textBox1.Text));
                    u.name = textBox2.Text; //u가 바뀌면 users 내용도 같이 바뀜
                    try //만약 빌린 책이 있다면 그 책에 대해서도 고객의 이름을 바꿔줘야 함
                    {
                        //해당 고객이 1권 혹은 여러 권 빌릴 수 있으므로 List 사용
                        List<Book> bookList = (from item in DataManager.Instance.books 
            ...
            button3.Click += (o, e) =>
            {
                ...
                {
                    User u = DataManager.Instance.users.Single(item => item.id.Equals(textBox1.Text));
                    DataManager.Instance.users.Remove(u);//얕은 복사이므로 Remove를 써도 그 위치가 지워짐
                    RefreshScreen();
                    DataManager.Instance.Save();
                }
                catch (Exception ex)
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (DataManager.Instance.users.Exists(item=>item.id.Equals(textBox1.Text)))
            ...
            else
            {
                ...
                DataManager.Instance.users.Add(newUser);
                RefreshScreen();
                DataManager.Instance.Save();
            }
        }
        ...
        public void RefreshScreen()
        {
            ...
            {
                dataGridView1.DataSource = DataManager.Instance.users;
