...
namespace BookManager
{
    public partial class Form1 : Form, IRefresh
    {
        ...
        public Form1()
        {
            ...
            BtnEvent borrowEvent = delegate (object s, EventArgs e) //대여
            {
                ...
                else
                {
                    try
                    {
                        ...
                        Book book = DataManager.Instance.books.Single(item => item.isbn.Equals(textBox1.Text));
                        if(book.isBorrowed)
                        ...
                        else
                        {
                            try
                            {
                                User u = DataManager.Instance.users.Single(item => item.id.Equals(textBox3.Text));
                                book.userId = u.id;
            ...
            BtnEvent returnEvent = (s, e) => //반납
            {
                ...
                else
                {
                    try
                    {
                        Book book = DataManager.Instance.books.Single
        ...
        {
            label1.Text = "전체 도서 수 : " + DataManager.Instance.books.Count; //이 때 Load() 호출됨
            label2.Text = "전체 회원 수 : " + DataManager.Instance.users.Count;
            label3.Text = "대출 중인 도서의 수 : ";
            //label3.Text += DataManager.books.Where(item => item.isBorrowed).Count();
            label3.Text += DataManager.Instance.books.Where(checkIsBorrowed).Count();
            label4.Text = "연체 중인 도서의 수 : ";
            label4.Text += DataManager.Instance.books.Where(delegate (Book item)
            {
            ...
            if(DataManager.Instance.books.Count>0)
                dataGridView1.DataSource = DataManager.Instance.books;
            dataGridView2.DataSource = null;
            if (DataManager.Instance.users.Count > 0)
                dataGridView2.DataSource = DataManager.Instance.users;
        }
